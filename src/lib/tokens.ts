...
export function resolveDesignTokens(tokens?: Tokens | null, site?: SiteSettings | null): {
  ...
  if (Array.isArray(tokens?.typographyScale)) {
    for (const item of tokens.typographyScale) {
      ...
      const fallback = typography[item.token] ?? DEFAULT_TYPOGRAPHY['body-md']
      typography[item.token] = {
        fontSize: item.fontSize || fallback.fontSize,
        lineHeight: item.lineHeight || fallback.lineHeight,
        fontWeight: item.fontWeight || fallback.fontWeight,
      }
    }
  }
  ...
  if (Array.isArray(tokens?.buttonStyles)) {
    for (const item of tokens.buttonStyles) {
      if (!item?.token) continue
      const fallback = buttons[item.token] ?? {}
      buttons[item.token] = {
        background: item.background || fallback.background,
        text: item.text || fallback.text,
        border: item.border || fallback.border,
        hoverBackground: item.hoverBackground || fallback.hoverBackground,
        hoverText: item.hoverText || fallback.hoverText,
        shadow: item.shadow || fallback.shadow,
      }
    }
  }
  ...
  for (const [token, value] of Object.entries(spacing)) {
    cssVariables[`--space-${token}`] = value
  }

  for (const [token, value] of Object.entries(typography)) {
    cssVariables[`--font-${token}-size`] = value.fontSize
    if (value.lineHeight) cssVariables[`--font-${token}-line`] = value.lineHeight
    if (value.fontWeight) cssVariables[`--font-${token}-weight`] = value.fontWeight
  }

  for (const [token, value] of Object.entries(radii)) {
    cssVariables[`--radius-${token}`] = value
  }
  ...
  for (const [token, value] of Object.entries(buttons)) {
    if (value.background) cssVariables[`--btn-${token}-bg`] = value.background
    if (value.text) cssVariables[`--btn-${token}-text`] = value.text
    if (value.border) cssVariables[`--btn-${token}-border`] = value.border
    if (value.hoverBackground) cssVariables[`--btn-${token}-hover-bg`] = value.hoverBackground
    if (value.hoverText) cssVariables[`--btn-${token}-hover-text`] = value.hoverText
    if (value.shadow) cssVariables[`--btn-${token}-shadow`] = value.shadow
  }
